...
class BasicDBTest(unittest.TestCase):
    ...
    def test_04_join_main(self):
        ...
        sql = 'select f.name,a.ivorn from frbs f join authors a on (f.author_id = a.id)'
        self.assertEqual(3, self.cursor.execute(sql))

        # Test join frbs and observations
        sql = 'select f.name,o.telescope,o.utc from frbs f join observations o on (f.id = o.frb_id)'
        self.assertEqual(4, self.cursor.execute(sql))

        # Test join observations and radio_observations_params
        sql = 'select o.telescope,o.utc,rop.settings_id,rop.raj,rop.decj from observations o join radio_observations_params rop on (o.id = rop.obs_id)'
        self.assertEqual(4, self.cursor.execute(sql))

        # Test join radio_observations_params and radio_measured_params
        sql = 'select rop.settings_id,rop.raj,rop.decj,rmp.voevent_ivorn from radio_observations_params rop join radio_measured_params rmp on (rop.id = rmp.rop_id)'
        self.assertEqual(4, self.cursor.execute(sql))

        # Test all main tables (frbs, observations, radio_observations_params and radio_measured_params)
        sql = 'select f.name,o.telescope,o.utc,rop.settings_id,rop.raj,rop.decj,rmp.voevent_ivorn from frbs f join observations o on (f.id = o.frb_id) join radio_observations_params rop on (o.id = rop.obs_id) join radio_measured_params rmp on (rop.id = rmp.rop_id)'
        self.assertEqual(4, self.cursor.execute(sql))

    def test_05_join_secondary(self):
        sql = 'select rmp.voevent_ivorn,ri.title from radio_measured_params rmp join radio_images_have_radio_measured_params rihrmp on (rmp.id = rihrmp.rmp_id) join radio_images ri on (rihrmp.radio_image_id = ri.id)'
        self.assertEqual(8, self.cursor.execute(sql))

        sql = 'select f.name,fn.note from frbs f join frbs_notes fn on (f.id = fn.frb_id)'
        self.assertEqual(2, self.cursor.execute(sql))

        sql = 'select o.telescope,o.utc,ont.note from observations o join observations_notes ont on (o.id = ont.obs_id)'
        self.assertEqual(1, self.cursor.execute(sql))

        sql = 'select rop.settings_id,rop.raj,rop.decj,ropn.note from radio_observations_params rop join radio_observations_params_notes ropn on (rop.id = ropn.rop_id)'
        self.assertEqual(1, self.cursor.execute(sql))

        sql = 'select rmp.voevent_ivorn,rmpn.note from radio_measured_params rmp join radio_measured_params_notes rmpn on (rmp.id = rmpn.rmp_id)'
        self.assertEqual(1, self.cursor.execute(sql))

        sql = 'select f.name,p.reference from frbs f join frbs_have_publications fhp on (f.id = fhp.frb_id) join publications p on (fhp.pub_id = p.id)'
        self.assertEqual(5, self.cursor.execute(sql))

        sql = 'select o.telescope,o.utc,p.reference from observations o join observations_have_publications ohp on (o.id = ohp.obs_id) join publications p on (ohp.pub_id = p.id)'
        self.assertEqual(8, self.cursor.execute(sql))

        sql = 'select rop.settings_id,rop.raj,rop.decj,p.reference from radio_observations_params rop join radio_observations_params_have_publications rophp on (rop.id = rophp.rop_id) join publications p on (rophp.pub_id = p.id)'
        self.assertEqual(8, self.cursor.execute(sql))

        sql = 'select rmp.voevent_ivorn,p.reference from radio_measured_params rmp join radio_measured_params_have_publications rmphp on (rmp.id = rmphp.rmp_id) join publications p on (rmphp.pub_id = p.id)'
        self.assertEqual(8, self.cursor.execute(sql))
    ...
    def test_06_insert_delete_main(self):
        ...
        sql = "DELETE FROM authors WHERE ivorn = %s"
        self.assertEqual(1, self.cursor.execute(sql, 'ivo://unknown2'))
        ...
        sql = "DELETE FROM radio_observations_params WHERE obs_id = %s AND author_id = %s AND settings_id = %s AND raj = %s AND decj = %s"
        self.assertEqual(1, self.cursor.execute(sql, (1,1,'mycustomsettings','00:00:00','00:00:00')))

        sql = "INSERT INTO radio_measured_params (rop_id,author_id,voevent_ivorn,voevent_xml) VALUES (%s,%s,%s,%s)"
        self.assertEqual(1, self.cursor.execute(sql, (1,1,'ivo://unknown_new','')))
        sql = "DELETE FROM radio_measured_params WHERE voevent_ivorn = %s"
